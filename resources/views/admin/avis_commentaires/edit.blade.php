<x-admin-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('admin.avis_commentaires.update', $avisCommentaire->id) }}" method="POST" class="bg-white shadow-xl rounded-lg p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="number" name="note" id="note" class="w-full p-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-300 ease-in-out" min="1" max="5" value="{{ $avisCommentaire->note }}">
            </div>

            <div class="flex justify-between">
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-semibold rounded-lg shadow-lg hover:bg-gradient-to-r hover:from-blue-600 hover:to-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-200 transition-all duration-300 ease-in-out">
                    Mettre à jour
                </button>
